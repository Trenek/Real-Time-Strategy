...
void lose(struct VulkanTools *vulkan, enum state *state) {
    ...
    struct Textures *texture = loadTextures(&textureDesc, &vulkan->graphics, sizeof(texturePath) / sizeof(const char *), texturePath);
    ...
    *model[0].instance = (struct instance){
    ...
    while (*state == LOSE && !glfwWindowShouldClose(vulkan->window)) {
        glfwPollEvents();
    ...
    unloadTextures(vulkan->graphics.device, sizeof(texturePath), texture, textureDesc);
}
