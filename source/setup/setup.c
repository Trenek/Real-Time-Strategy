...
struct VulkanTools setup(uint32_t modelQuantity, struct ModelBuilder modelBuilder[modelQuantity]) {
    ...
    vulkan.window = createWindow(&vulkan.framebufferResized);
    ...
    vulkan.modelQuantity = modelQuantity;
    vulkan.model = malloc(sizeof(struct Model) * modelQuantity);
    for (uint32_t i = 0; i < modelQuantity; i += 1) {
        vulkan.model[i] = createModels(modelBuilder[i], vulkan.device, vulkan.physicalDevice, vulkan.surface, vulkan.renderPass, vulkan.msaaSamples, vulkan.uniformBuffers, vulkan.transferCommandPool, vulkan.transferQueue);
    }
...
void cleanup(struct VulkanTools vulkan) {
    ...
    for (uint32_t i = 0; i < vulkan.modelQuantity; i += 1) {
        destroyModels(vulkan.device, vulkan.model[i]);
    }
    free(vulkan.model);
